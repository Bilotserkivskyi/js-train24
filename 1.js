...
function task1() {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (Object.keys(data).length > 0) {
          resolve(data); // Виклик resolve з переданими даними
        } else {
          reject(new Error("Об'єкт пустий")); // Виклик reject з новим об'єктом помилки
        }
      }, timeout);
    });
  }
  ...
  getData(inputData)
    .then((result) => {
      console.log("Отримані дані:", result); // Обробка успішного результату
    })
    .catch((error) => {
      console.error("Помилка:", error.message); // Обробка помилки
    })
    .finally(() => {
      console.log("Завдання 1 завершено"); // Виконується незалежно від успіху чи помилки
    });
}
